...
interface IConvexStakingWrapperFrax {
    ...
    function rewards(
        ...
        external
        view
        returns (address reward_token, address reward_pool, uint256 reward_integral, uint256 reward_remaining);
...
interface IBunniGauge {
    ...
    function initialize(
        address _lp_token,
        uint256 relative_weight_cap,
        address _voting_escrow_delegation,
        address _admin,
        bytes32 _position_key
    ) external;
...
contract FraxUnifiedFarmTemplate is Owned, ReentrancyGuard {
    ...
    uint256 public lock_time_for_max_multiplier = 1 * 1095 * 86_400; // 3 years
    ...
    // Reward period
    uint256 public constant rewardsDuration = 604_800; // 7 * 86400  (7 days)
    ...
    modifier onlyByOwnGov() {
        require(
            msg.sender == owner || msg.sender == 0x8412ebf45bAC1B340BbE8F318b928C466c4E39CA,
    ...
    function rewardRates(uint256 token_idx) public view returns (uint256 rwd_rate) {
        ...
        if (gauge_controller_address != address(0)) {
            rwd_rate =
                (IFraxGaugeController(gauge_controller_address).global_emission_rate() *
    ...
    function maxLPForMaxBoost(address account) external view returns (uint256) {
        return
            (veFXS.balanceOf(account) * MULTIPLIER_PRECISION * MULTIPLIER_PRECISION) /
        ...
        for (uint256 i = 0; i < gaugeControllers.length; i++) {
            ...
            if (gauge_controller_address != address(0)) {
                if (force_update || (block.timestamp > last_gauge_time_totals[i])) {
                    ...
                    last_gauge_relative_weights[i] = IFraxGaugeController(gauge_controller_address)
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) external onlyTknMgrs(tokenAddress) {
        ...
        if ((isRewTkn && rewardManagers[tokenAddress] == msg.sender) || (!isRewTkn && (msg.sender == owner))) {
        ...
        onlyByOwnGov
    {
        ...
        require(
            (_misc_vars[1] >= 0) && (_misc_vars[2] >= 0) && (_misc_vars[3] >= 0) && (_misc_vars[5] >= 0),
            "Must be >= 0"
        );
...
contract FraxUnifiedFarm_ERC20 is FraxUnifiedFarmTemplate {
    ...
    function lockAdditional(
        bytes32 kek_id,
        uint256 addl_liq
    ) public nonReentrant updateRewardAndBalanceMdf(msg.sender, true) {
    ...
    function _withdrawLocked(
        ...
        if (claim_rewards || !collectRewardsOnWithdrawalPaused) {
...
contract FraxUnifiedFarm_ERC20_Convex_frxETH is FraxUnifiedFarm_ERC20 {
    ...
    function fraxPerLPToken() public view override returns (uint256) {
        ...
        // ============================================
        {
            ...
            uint256 frxETH_usd_val_per_lp_e8 = (frxETH_in_pool * uint256(getLatestETHPriceE8())) /
