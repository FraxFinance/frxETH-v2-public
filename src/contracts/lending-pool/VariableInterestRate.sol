...
/// @param suffix The suffix of the contract name
/// @param vertexUtilization The utilization at which the slope increases
/// @param vertexRatePercentOfDelta The percent of the delta between max and min, defines vertex rate
/// @param minUtil The minimum utilization wherein no adjustment to full utilization and vertex rates occurs
/// @param maxUtil The maximum utilization wherein no adjustment to full utilization and vertex rates occurs
/// @param zeroUtilizationRate The interest rate (per second) when utilization is 0%
/// @param minFullUtilizationRate The minimum interest rate at 100% utilization
/// @param maxFullUtilizationRate The maximum interest rate at 100% utilization
/// @param rateHalfLife The half life parameter for interest rate adjustments
struct VariableInterestRateParams {
    string suffix;
...
contract VariableInterestRate is IInterestRateCalculator {
    ...
    /// @param _params The contructor VariableInterestRateParams params
    constructor(VariableInterestRateParams memory _params) {
        suffix = _params.suffix;
        MIN_TARGET_UTIL = _params.minUtil;
        MAX_TARGET_UTIL = _params.maxUtil;
        VERTEX_UTILIZATION = _params.vertexUtilization;
        ZERO_UTIL_RATE = _params.zeroUtilizationRate;
        MIN_FULL_UTIL_RATE = _params.minFullUtilizationRate;
        MAX_FULL_UTIL_RATE = _params.maxFullUtilizationRate;
        RATE_HALF_LIFE = _params.rateHalfLife;
        VERTEX_RATE_PERCENT = _params.vertexRatePercentOfDelta;

        // Check constants
        require((UTIL_PREC - _params.vertexUtilization) > 0, "UTIL_PREC - VERTEX_UTILIZATION cannot be 0.");
    }
    ...
    /// @notice The ```_getFullUtilizationInterest``` function calculate the new maximum interest rate, i.e. rate when utilization is 100%
    ...
    function _getFullUtilizationInterest(
        uint256 _deltaTime,
    ...
    function getNewRate(
        ...
        _newFullUtilizationInterest = _getFullUtilizationInterest(
            _deltaTime,
            _utilization,
