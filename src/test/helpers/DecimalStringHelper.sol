...
library DecimalStringHelper {
    function decimalStringI256(
        int256 number,
        uint8 decimals,
        bool isPercent
    ) public pure returns (string memory _rtnVal) {
        if (number < 0) {
            return string.concat("-", decimalString(uint256(-1 * number), decimals, isPercent));
        } else {
            return decimalString(uint256(number), decimals, isPercent);
        }
    }
